...
- **用户 ID 与听歌统计**：6 位数字 ID、听歌时长 / 播放次数，**只保存在本机**；
  换机 / 重装后用「备份与恢复」导出、导入备份文件即可带走
- **备份与恢复**：导出 / 导入备份文件，可选是否包含登录信息与壁纸图片，并可清除图片缓存
- **更新日志**：应用内查看各版本改动
- **问题反馈**：调用系统邮件 App 发送（收件人已预填），并自动附带运行环境
- 中英双语

### 本地统计与反馈

听歌统计没有任何云端同步：不需要 Gitee 仓库、不需要令牌，构建也不读取任何私密配置
（`local.properties` 里只剩 `sdk.dir`）。换机搬运统计请使用设置页的「备份与恢复」。

问题反馈通过系统邮件 App 发送到 `57844458@qq.com`（收件人已预填）；
设备上没有邮件 App 时不会崩，内容会自动复制到剪贴板，由用户自行粘贴。

### 「我的」页面模块

大标题「我的」→ 账号卡片 → 听歌统计卡片（听歌时长 / 播放次数 / 6 位用户 ID）→
已下载 → **交流群** → **自愿赞助** → 版本号页脚。
