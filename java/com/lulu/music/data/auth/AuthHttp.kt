...
class AuthHttpExchange(
    ...
    val location: String,
    /** HTTP 状态码；只用于登录诊断（判断这一步到底是 302 还是 200/4xx），不参与业务分支。 */
    val status: Int = 0,
) {
...
suspend fun authExchange(request: Request, followRedirects: Boolean = true): AuthHttpExchange =
    withContext(Dispatchers.IO) {
        try {
            val client = if (followRedirects) Http.client else noRedirectClient
...
/**
 * 不跟随 302 的客户端。原先每跳都用 `Http.client.newBuilder()...build()` 现造一个 ——
 * 每次新建都带一个全新的连接池与线程池，跳转链最多 6 跳就是 6 次 TLS 握手，
 * 没有任何好处。这里改成共享一个实例（`OkHttpClient` 本身就是设计成共享复用的）。
 */
private val noRedirectClient: okhttp3.OkHttpClient by lazy {
