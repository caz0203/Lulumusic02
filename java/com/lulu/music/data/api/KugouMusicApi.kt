...
object KugouMusicApi {
    ...
    suspend fun lyric(hash: String, duration: Double): String {
        val candidate = lyricCandidate(hash, duration) ?: return ""
        val data = downloadLyric(candidate, fmt = "lrc") ?: return ""
        return String(data, Charsets.UTF_8)
    }

    /**
     * 逐字歌词入口：有 KRC 就用 KRC，否则退回 [lyric] 的整行 LRC（`words` 全空）。
     *
     * KRC 与 LRC 走**同一个** `lyrics.kugou.com/download`，只是 `fmt` 从 `lrc` 换成 `krc`；
     * 候选（id + accesskey）来自同一次 `lyrics.kugou.com/search`，所以这里只搜一次。
     * `fmt=krc` 拿回来的是 base64 包着的 `krc1` 二进制，解码在 [KrcCodec]。
     *
     * 实测：不需要登录也能拿到 KRC，但**不是每个候选都有**（下架/冷门歌词只有 lrc），
     * 所以任何一步失败都必须干净地退回整行 LRC，绝不能返回空歌词。
     */
    suspend fun lyricWithWords(hash: String, duration: Double): List<LyricLine> {
        // 与网易云/QQ 一致：把「测试禁用网络」的闸门放在建连之前。
        Http.guardNetwork()
        if (hash.isEmpty()) return emptyList()
        val candidate = runCatching { lyricCandidate(hash, duration) }.getOrNull()
        if (candidate != null) {
            val krc = runCatching { downloadLyric(candidate, fmt = "krc") }.getOrNull()
            val wordLevel = KrcParser.parse(KrcCodec.decode(krc))
            if (LyricFallback.hasWordTiming(wordLevel)) return wordLevel
            // KRC 缺失或没有逐字数据：退回**同一个候选**的 lrc，不再重新搜一次。
            val lrc = runCatching { downloadLyric(candidate, fmt = "lrc") }.getOrNull()
            if (lrc != null) {
                return LyricFallback.best(LyricParser.parse(String(lrc, Charsets.UTF_8)), wordLevel)
            }
        }
        // 连候选都没搜到：沿用旧的整体流程（它会自己再搜一次），行为与改动前逐字一致。
        return LyricParser.parse(lyric(hash, duration))
    }

    /** 歌词候选（id + accesskey）。对应原来的搜索 + 取首条。 */
    private suspend fun lyricCandidate(hash: String, duration: Double): KugouLyricCandidate? {
        ...
        val id = first.opt("id") ?: return null
        val accessKey = first.opt("accesskey") ?: return null
        return KugouLyricCandidate(id = string(id), accessKey = string(accessKey))
    }

    /** 下载歌词原始字节（base64 已解）。[fmt] 为 `lrc` 或 `krc`。 */
    private suspend fun downloadLyric(candidate: KugouLyricCandidate, fmt: String): ByteArray? {
        ...
        val djson = runCatching { getJson(downloadURL, browserUA) }.getOrNull() ?: return null
        val content = djson.opt("content") as? String ?: return null
        return runCatching { Base64.getDecoder().decode(content.replace("\n", "")) }.getOrNull()
    }

    private data class KugouLyricCandidate(val id: String, val accessKey: String)
