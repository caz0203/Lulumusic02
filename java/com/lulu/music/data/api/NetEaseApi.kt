...
object NetEaseApi {
    ...
    suspend fun lyric(id: Long): String? {
        val json = lyricPayload(id, wordLevel = false)
    ...
    ///
    /// [yrc] 是逐字歌词（YRC 明文），只有 [lyricWithWordLevel] 会填充；旧调用方拿到的是 null。
    data class LyricResult(val lrc: String?, val tlyric: String?, val yrc: String? = null)
     ...
     * 逐字歌词 + 整行歌词 + 翻译（一次请求拿全）。
     *
     * `yv/ytv/yrv = -1` 让 `/api/song/lyric` 在同一个响应里额外返回 `yrc` 块；`lrc` / `tlyric`
     * 照旧一起返回，所以「有逐字用逐字、没有退回整行」不需要第二次请求。
     * 已用真实响应确认：weapi 通道带上这三个参数后 `lrc` 内容不变、`yrc` 出现。
     */
    suspend fun lyricWithWordLevel(id: Long): LyricResult {
        val json = lyricPayload(id, wordLevel = true)
        val lrc = json.optMap("lrc")?.stringOrNull("lyric")
        val tlyric = json.optMap("tlyric")?.stringOrNull("lyric")
        val yrc = json.optMap("yrc")?.stringOrNull("lyric")
        return LyricResult(lrc = lrc, tlyric = tlyric, yrc = yrc)
    }

    /**
     * 逐字歌词入口：有真逐字数据就用真的，否则退回整行 LRC（`words` 全空）。
     *
     * **整行歌词成功时绝不返回空**：`yrc` 缺失、解析不出词、甚至带 `yv` 的请求报错，
     * 都会退到 [lyricWithTranslation] 的整行结果。
     */
    suspend fun lyricWithWords(id: Long): List<LyricLine> {
        val result = try {
            lyricWithWordLevel(id)
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
            lyricWithTranslation(id)
        }
        return LyricFallback.best(
            lineLevel = LyricParser.parse(result.lrc.orEmpty(), result.tlyric),
            wordLevel = YrcParser.parse(result.yrc),
        )
    }

    /** 带 `yv/ytv/yrv` 与否的唯一分叉点；[lyric] / [lyricWithTranslation] 的参数与改动前逐字一致。 */
    private suspend fun lyricPayload(id: Long, wordLevel: Boolean): JSONObject {
        val payload = if (wordLevel) {
            mapOf("id" to id, "lv" to -1, "kv" to -1, "tv" to -1, "yv" to -1, "ytv" to -1, "yrv" to -1)
        } else {
            mapOf("id" to id, "lv" to -1, "kv" to -1, "tv" to -1)
        }
        return request("/api/song/lyric", payload, "weapi")
    }
